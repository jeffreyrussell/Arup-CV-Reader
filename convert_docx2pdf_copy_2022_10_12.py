...
def docx_to_pdf(data):
    ...
    temp_folder = os.path.join(output_folder) #STORE ALL RESUMES IN OUTPUT FOLDER
    ...
    for value in data['list_docx_files']:
        ...
        # temp_docx_path = os.path.join(temp_dir_list[dir_index], docx_name)
        temp_docx_path = os.path.join(output_folder, docx_name)

        if os.path.exists(docx_path):
            # if os.path.exists(temp_dir_list[dir_index]):
            # if os.path.exists(temp_dir_list):
                # copy2(src=docx_path, dst=temp_docx_path)  # create copy of file to avoid overwriting mod dates.
            copy2(src=docx_path, dst=output_folder)  # create copy of file to avoid overwriting mod dates.
            # else:
                # os.makedirs(temp_dir_list[dir_index])
                # copy2(src=docx_path, dst=temp_docx_path)  # create copy of file to avoid overwriting mod dates.
    ...
    #     print(f"\n-------- Segment {idx_1}/{len(temp_dir_list)} (***Watch computer for pop-ups from Word. Always select 'Yes' when asked to save, and select 'General' for confidentiality pop-ups.***):")
